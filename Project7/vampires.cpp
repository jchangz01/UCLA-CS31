...
private:
	...
	int m_row;
	int m_col;
	int condition; //to check if poisoned or not
	int psnd; //for poison effect
};
...
void Vampire::move()
{
	if (condition == 0)
	{
		int direction = randInt(0, 3); //generates a random direction for vampires
		if (attemptMove(*m_arena, direction, m_row, m_col))
		{
			if (m_arena->getCellStatus(m_row, m_col) == HAS_POISON) //checks for poison
			{
				condition++;
				psnd++;
	...
	else if (condition == 1) //if vampire is poisoned. . .
	{
		if (psnd % 2 == 0)
		{
...
string Player::move(int dir)
{
	if (dir == 0)
	{
		...
		{
			if (m_arena->numberOfVampiresAt(row(), col()) > 0)
			{
				setDead();
...
void Player::setDead()
{
	m_dead = true; //set player to dead
}
...
{
	return m_rows; //return number of rows in arena
}

int Arena::cols() const
{
	return m_cols; //return number of columns in arena
}
...
int Arena::numberOfVampiresAt(int r, int c) const
{
	...
	for (int v = 0; v < m_nVampires; v++)
	{
		if (m_vampires[v]->col() == c && m_vampires[v]->row() == r)
		{
			numVamps++;
		}
	}
...
void Arena::display(string msg) const
{
	...
	for (int v = 0; v < m_nVampires; v++)
	{
		for (int p = 2; p != 9; p++) //check for mutliple vampires
		{
			if (displayGrid[m_vampires[v]->row() - 1][m_vampires[v]->col() - 1] >= '9')
				break;

			else if ((int)displayGrid[m_vampires[v]->row() - 1][m_vampires[v]->col() - 1] - '0' == p)
			{
				displayGrid[m_vampires[v]->row() - 1][m_vampires[v]->col() - 1] = (m_vampires[v]->isDead() ? 
				'0' + p - 1 : '0' + p + 1);
				break;
			}
		}

		if (displayGrid[m_vampires[v]->row() - 1][m_vampires[v]->col() - 1] == 'V')
			displayGrid[m_vampires[v]->row() - 1][m_vampires[v]->col() - 1] = (m_vampires[v]->isDead() ? 'V' : '2');

		else if (!isdigit(displayGrid[m_vampires[v]->row() - 1][m_vampires[v]->col() - 1]))
			displayGrid[m_vampires[v]->row() - 1][m_vampires[v]->col() - 1] = (m_vampires[v]->isDead() ? '.' : 'V'); 
	} 
...
void Arena::moveVampires()
{
	for (int v = 0; v < m_nVampires; v++)
	{
		...
		else if (m_vampires[v]->isDead())
		{
			delete m_vampires[v];
			m_vampires[v] = m_vampires[m_nVampires - 1];
			v--; 
			m_nVampires--;
		}
	}
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
	if (a.numberOfVampiresAt(r - 1, c + 0) == 0 && r - 1 > 0)
	{
		bestDir = NORTH;
		if (a.numberOfVampiresAt(r - 1, c + 1) == 0 && a.numberOfVampiresAt(r - 1, c - 1) == 0
			&& a.numberOfVampiresAt(r - 2, c + 0) == 0)
		{
			return true;
		}
	}

	if (a.numberOfVampiresAt(r + 0, c + 1) == 0 && c + 1 <= a.cols())
	{
		bestDir = EAST;
		if (a.numberOfVampiresAt(r - 1, c + 1) == 0 && a.numberOfVampiresAt(r + 1, c + 1) == 0
			&& a.numberOfVampiresAt(r - 0, c + 2) == 0)
		{
			return true;
		}
	}

	if (a.numberOfVampiresAt(r + 1, c + 0) == 0 && r + 1 <= a.rows())
	{
		bestDir = SOUTH;
		if (a.numberOfVampiresAt(r + 1, c + 1) == 0 && a.numberOfVampiresAt(r + 1, c - 1) == 0
			&& a.numberOfVampiresAt(r + 2, c + 0) == 0)
	...
	if (a.numberOfVampiresAt(r + 0, c - 1) == 0 && c -1 > 0)
	{
		bestDir = WEST;
		if (a.numberOfVampiresAt(r + 1, c - 1) == 0 && a.numberOfVampiresAt(r - 1, c - 1) == 0
			&& a.numberOfVampiresAt(r - 0, c - 2) == 0) 
